...
def generateImg(result, path = "./"):
    ...
    plt.title('top5HighestGrowthIndiaYear')
    plt.xticks(rotation=45, ha="right")
    plt.savefig(os.path.join(path, 'top5HighestGrowthIndiaYear.png'))


if __name__ == "__main__":
    if(len(sys.argv)<3):
        print("Usage: spark-submit top5HighestGrowthIndiaYear.py <dataset dir> <year>")
        exit(0)
    dir = sys.argv[1]
    
    result = top5HighestGrowthIndiaYear(dir,sys.argv[2])
    print(result)
